...
import inspect
from hvdconfig import TF as hvdconfig
import time
import ntpath
...
def generate_horovodized_code(root_node, save_to_file, filename):
...
def add_horovod_initialization(root_node, idx):
    ...
    body = get_body_nodes(root_node)[0]
    return add_code(body, idx, copy(hvdconfig.configs))
...
def find_model_names(root_node):
...
def adapt_model_fit(root_node, model_names):
    ...
    def adapt_model_fit_generator_recursive(body, adapted, model_names):
        ...
        def adapt_keywords_model_fit_generator(keywords):
            ...
            adapted_callbacks = adapted_verbose = False
            ...
            adapted = adapted_callbacks and adapted_verbose and \
                        adapted_validation_steps and adapted_steps_per_epoch
...
def horovodize(path):
    ...
    name = "horovodized/horovodized_" + ntpath.basename(path)
    ...
    if len(model_names) == 0:
        print("Ooops! No models found! Aborting...")
        return
    ...
    print("Code adaptations: DONE")
    # Generate new python code
    generate_horovodized_code(root_node=node, save_to_file=True, filename=name)
