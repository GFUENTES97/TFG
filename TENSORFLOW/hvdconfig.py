...
## CODE ADAPTATIONS
    ...
    # hvd_distr_opt=Assign(targets=[Name(id='optimizer',ctx=Store())],value=Call(func=Attribute(value=Name(id='hvd',ctx=Load()),attr='DistributedOptimizer',ctx=Load()),args=[Name(id='optimizer',ctx=Load())],keywords=[keyword(arg='named_parameters',value=Call(func=Attribute(value=Name(id='model',ctx=Load()),attr='named_parameters',ctx=Load()),args=[],keywords=[]))]))
    # adapt loss & accuracy
    adapt_loss=[
